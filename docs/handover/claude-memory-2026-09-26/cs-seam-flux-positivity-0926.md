...
- **Vertex defect FIXED 09-26 (merge 6a3d051c):** the along-seam quadratic ghost resample (bvals_cc.cpp) has negative weights and a clamp: it makes dense ghosts of empty cells and E<KE ghosts. New keys <mesh>/cs_seam_resample = linear and cs_seam_rho_guard = 4 (default off, bitwise). FOFC does not cure it. RECOMMENDED cs set: positive + linear + guard 4; only positive has had the 12-rot dhj check so far.
