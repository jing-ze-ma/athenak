...
- **[B-STAR COLUMN PULSATION = NUMERICAL O(dt) anti-damping of the per-cell source (gamma linear in dt, zero at the production dt); ALI restores physical damping -> FINAL CANDIDATE whole column + rt_bottom_flux + ALI; column DONE (Ftop 0.9986, slow residual regrowth 26-turnover e-fold) + prod_whole STAGED for 8 GPUs, 18 h, NOT submitted (user)](bstar-column-pulsation.md)**
