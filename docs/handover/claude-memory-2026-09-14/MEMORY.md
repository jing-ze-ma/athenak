...
- **[He MODE = dt-LINEAR FORCING by the per-cell semi-implicit factor on the non-local exchange; no cheap switch (sub-cycle/re-sweep/clips all refuted); needs an implicit column solve (~1 week); PARKED; B star floor 0.6 % v_MLT](rt-source-dt-forcing.md)**
...
- **[IMPLICIT TRANSVERSE OPERATOR WORKING + UNIFIED rad_sts_all (ported to box); SPEED PACKAGE: rad_sts_split = accuracy, rad_sts_once = worse, unported](implicit-transverse-raddiff-plan.md) — PCG deferred (1.25x cost)**
