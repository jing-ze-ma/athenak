...
**CURRENT STATE, 2026-09-15 ~19:00, VIPER.** Two wide-box productions RUNNING (binary rg-box-pcr 3596f5b3, md5 a1ef8822: mode 3 + PCR nseg 32, ADI lod2 with the horizontal operator everywhere, sweep skip, residual check, open top): B-star prod_w4 (bench/bstar_fecz/prod_w4, chain 11712113-5, 393 Mm = 4x wider, 4:1 cells) at 16+ turnovers, CLEAN, 220 ms/cycle, 40 turnovers inside link 1 (~04:00 09-16); convection FREE of the box (30 downdrafts, 139 Mm = 2.1 layer thicknesses), plaid cured, light curve 1.1 mmag detrended; caveat: a GROWING box-quantized surface f-MODE ((2,2), sqrt(gk) to 1 %, e-fold 9.3 turnovers, v_z already > v_MLT at tau 30): physical vs O(dt)-driven undecided -> dt test submitted (plaid/fmode arms) [[bstar-prod-w4-plan]]. He box_w4 (bench/hestar_fecz/box_w4, chain 11712116-8) at 8+ turnovers, ALIVE but a TRANSONIC WIND through the open top drains 2.4 % of M0 per turnover uniformly (photosphere sank 0.28 H_p, dt 0.109 -> 0.06 from the lid outflow velocity); convection free of the box; USER DECISION pending: feed the wind (bottom inflow bc_mode_bot 5, COMMITTED 3a477831, 726 cm/s) + a lid measure (lower top to tau 0.05-0.1 or sponge) at the link-2 break, or a different top ([[he-box-m3-staging]]). Old prod_m3 (98 Mm) STOPPED at 12 turnovers (rst 00012) after the plaid + taper 1/3 runaway. Science page v5 (both productions): https://claude.ai/artifact/HQ5bHyDW67yShdAygahjBS; its source + all analysis scripts are in bench/analysis_0915/ (not the scratchpad). Handover: docs/handover/HANDOVER-2026-09-15.md + addendum A (main 0955b423, wt_pap 3e505726); addendum B owed for the evening. Commits today on rg-box-pcr (wt_m3acc, UNPUSHED) through 235b05f0: bottom-flux fix, bvals window, transpose, restart fix, halo fence, halo_every, sweep skip/warm/once, ADI lod2, open top+sponge, rescheck (default on), redpar, inflow, hybrid column (PCR 1.09x). Rules: [[no-heavy-monitoring]], [[sbatch-snapshots-script]], [[verify-spectral-verdicts]].

**NEXT STEPS (09-15 evening):** (1) B star: when link 1 ends (~40 turnovers) regenerate the science page (scripts in the session scratchpad bstar_page/, w4check/) and read the wave-train identification; (2) He: decide the wind question, then at the link-2 break restart from the newest rst with bc_mode_bot = 5 (bc_inflow_time one turnover) + the lid measure, gate on dM/dt ~ 0 and tau=2/3 within 2 cells; (3) port mode 3 + fixes to rg/dhj (rg needs rad_flux_inner -> rt_bot_flux; dhj ck path not covered by the mode-3 gate); (4) the short-characteristics lid (next physics project); (5) deferred: exact restart (primitives in the rst), PCG/multigrid, merge order (lhlld vs implicit-transverse-raddiff first, discrepancy flagged); (6) push rg-box-pcr.
