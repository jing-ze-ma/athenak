...
- **[SPHERICAL TWO-STREAM DIFFUSION-LIMIT ERROR = 1 - H_T/(2r) (verified 09-17): hand tau > 20 to diffusion on spheres; variable-Eddington rewrite later](two-stream-sphere-diffusion-error.md)**
...
**CURRENT STATE, 2026-09-17 18:40, VIPER.** PRODUCTIONS: B star prod_w7 at 37/40 turnovers (24-h chain running, statistically steady since 16; ends tonight); He box_w8 at 26 turnovers (24-h chain; reached 0.9 v_MLT at 13); box_w9 DELETED. Page v7 v3 https://claude.ai/artifact/SLgYFj9JDBUsNqxj5UYgPJ. GLOBAL 4 Msun PRESUPERNOVA He STAR (branch he4-presn-global, bench/wt_he4, 36 commits, unpushed): all code in place (r^2 two-stream, box physics in red_giant, ic_profile reader + spherical 1-D column, cs implicit ADI transverse incl. multi-block panels + 2-node MPI, seam-halo fix); 4 bugs fixed. BLOCKER: the spherical two-stream carries 1 - H_T/(2r) of the true flux in the diffusion limit (unit test tests_r2/thick, 1e-4) -> every smoke arm dies < 0.4 turnover; tau handover 20/300 made it worse (double count with the MLT closure). NEXT = moment-equation rewrite with a variable Eddington factor (user decision). Handover docs/handover/HANDOVER-2026-09-17-B.md.

**NEXT STEPS (09-18):** (1) user decides on the moment-equation rewrite of the spherical two-stream (sphericity in the zeroth moment, variable Eddington factor; gates tests_r2/rg1d + tests_r2/thick), then fix the MLT-closure double count, rerun arms B/D/BH/DH on the smoke grid, then production (2x2 or 4x4 blocks/panel). (2) B star: final analysis at 40 turnovers (light curve, f-mode split fit). (3) Check which dhj/rg spherical-polar productions ran with rt_rad_force (transverse term bug). (4) bvals_fc seam defects for cs MHD. (5) Push he4-presn-global / cs branches after review.
