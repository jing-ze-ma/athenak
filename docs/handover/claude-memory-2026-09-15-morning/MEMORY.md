...
**CURRENT STATE, 2026-09-14 (~05:00), VIPER.** B-star production prod_taper RUNNING 11701870 (uncut, taper+force, operator, FIXED sweep, column ic_fecz_rt_relaxed_fix_taper.txt, 20 turnovers, ~7 h; safety link 11701871); prod_uncut cancelled at 6.9 turnovers (data kept). polar-average-perf = fork = d8f00d49 (RT cut/limiter/sweep fixes incl. ck, HLLD tol; red giant radial v changes 25 % rms with the new defaults; rg/dhj should set rt_use_cons=true). UNMERGED branches: rg-box-convection (wt_rgbox, HEAD 86cc1b0b: operator, unified RKL1, restart fix, taper/force, all RT fixes, solver fix, speed switches, rt_relax_sub/rt_outer_iter experiments), implicit-transverse-raddiff (main tree, 3cf3161d+61662b2a), lhlld (wt_lhlld 15e9f790: LHLLD aligned, Balsara vortex, HLLD tol). He project PARKED (dt-linear forcing by the per-cell semi-implicit RT factor; needs an implicit column solve). sp_mhd_nopole 11639437 ENDED 09-15 ~02:45 on its wall clock at rotation 173.8/283 (rst dhj.00348, restart-aware submit.sh; needs 2 more 24 h links, user decision); cs_mhd_prod3 DONE (283 rotations). Details: [[bstar-box-growth-and-production]], [[rt-source-dt-forcing]], [[two-stream-layer-stagger-fix]], [[implicit-transverse-raddiff-plan]], [[lhlld-solver]].

**NEXT STEPS (09-14):** (1) prod_taper second-half analysis when done (growth.py / compare_*.py templates in rt2turn_topcut/analysis); (2) sp_mhd_nopole links (user); (3) rg/dhj rerun with the new defaults + rt_use_cons=true; (4) scope/implement the implicit column solve for the two-stream source (unparks He); (5) merge order: lhlld -> implicit-transverse-raddiff -> rg-box-convection onto polar-average-perf with the bitwise gates.
...
- **[B-STAR COLUMN PULSATION = NUMERICAL O(dt) anti-damping of the per-cell source (gamma linear in dt, zero at the production dt); ALI restores physical damping -> FINAL CANDIDATE whole column + rt_bottom_flux + ALI; prod_whole RESTAGED (tapered horizontal operator, 8 GPUs 19 h), NOT submitted (user); MODE-3 RING SOLVED = horizontal column-to-column mode from independent exact columns without horizontal RT coupling; quiet with exchange everywhere (~5x cost); taper test running](bstar-column-pulsation.md)**
...
- **[IMPLICIT TRANSVERSE OPERATOR WORKING + rad_blend_transverse: horizontal RT transport was OFF above tau 100 in all runs; on everywhere = 3x cost (thin region stiff); PCG relevant again](implicit-transverse-raddiff-plan.md) — PCG deferred (1.25x cost)**
