...
OUT="output1/dt=47.0 output2/dt=1.0e30 output3/dt=940.0 output4/dt=1.0e30
 output5/dt=47.0 problem/rt_profile_dt=235.25 problem/rt_surface_dt=470.5
 problem/face_budget=0"
d=$1; shift
A=$B/tests_r13/$d; mkdir -p $A; cd $A
RST=$(ls -t $A/rst/*.rst 2>/dev/null | head -1)
echo "=========== ARM $d : $@"
srun -n 1 $X ${RST:+-r $RST} -i $IN -t 00:13:00 $ONED $GNR $OUT time/tlim=14115.0 \
  ...
  | tail -60
rm -rf $A/bin $A/cbin_hydro_w_2; ls -t $A/rst/*.rst 2>/dev/null | tail -n +2 | xargs -r rm -f
echo "=========== ARM $d DONE"
