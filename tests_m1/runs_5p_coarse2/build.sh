...
# m1-coarse2 HEAD, or the working tree if REV=WT) into src_<tag>, build, copy to bin/
set -e
W=/viper/ptmp2/jinma/coarse2_0925; S=$W/src_$1; B=$W/b_$1_$2_$3; R=${REV:-HEAD}; WT=/viper/ptmp2/jinma/wt_coarse2
if [ ! -d $S ]; then
  mkdir -p $S
  if [ "$R" = WT ]; then
