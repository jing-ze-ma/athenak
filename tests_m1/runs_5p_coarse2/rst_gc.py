"""Restart gate of implicit_precond = mg_gc (runs_5p_coarse2): the CI restart test
tst/test_suite/rad_m1/test_rad_m1_restart_mpicpu.py with implicit_precond = mg_gc,
implicit_mg_levels = 3, 2 bands in x2 and implicit_bcg_rho_direct on top (Eddington and
vet_sc), with a given binary.

  ATHENAK_M1_DATA=... python3 rst_gc.py <athena (CPU, MPI, box_convection)> <rundir>
"""
import glob
import os
import sys
...
inp = os.path.join(rd, "he_slab_m1_gc.athinput")
...
m1.INPUT = inp
MG = ["rad_m1/implicit_precond=mg_gc", "rad_m1/implicit_mg_levels=3",
      "rad_m1/implicit_gc_bands2=2", "rad_m1/implicit_bcg_rho_direct=true"]
ok = True
