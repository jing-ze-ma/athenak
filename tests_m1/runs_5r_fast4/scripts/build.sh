...
# m1-fast4 HEAD, or the working tree if REV=WT) into src_<tag>, build, copy to bin/
set -e
W=/viper/ptmp2/jinma/fast4_0925; S=$W/src_$1; B=$W/b_$1_$2_$3; R=${REV:-HEAD}; WT=/viper/ptmp2/jinma/wt_fast4
if [ ! -d $S ]; then
  mkdir -p $S
  if [ "$R" = WT ]; then
    git -C $WT archive HEAD | tar -x -C $S; rm -rf $S/kokkos
    (cd $WT && { git diff --name-only HEAD; git ls-files --others --exclude-standard src; } | xargs -r cp --parents -t $S/)
  else
