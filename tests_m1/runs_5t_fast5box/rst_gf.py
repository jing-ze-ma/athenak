"""Restart gate of implicit_precond = mg_gf (runs_5t_fast5box): the CI restart test
tst/test_suite/rad_m1/test_rad_m1_restart_mpicpu.py with implicit_precond = mg_gf,
implicit_mg_levels = 3, implicit_gf_modes = 2 (Eddington and
...
inp = os.path.join(rd, "he_slab_m1_gf.athinput")
txt = open(m1.INPUT).read().replace(
    "<rad_m1>\n", "<rad_m1>\nimplicit_mg_levels = 2\nimplicit_mg_halo = true\n"
    "implicit_gf_modes = 2\n", 1)
open(inp, "w").write(txt)
m1.INPUT = inp
MG = ["rad_m1/implicit_precond=mg_gf", "rad_m1/implicit_mg_levels=3",
      "rad_m1/implicit_gf_modes=2"]
ok = True
