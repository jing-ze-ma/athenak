#!/bin/bash -l
# T-bit CPU gates of m1-sp (ref = rt-integration 90b01f2f, new = m1-sp working tree)
W=/viper/ptmp2/jinma/s1_0924; R=$W/run.sh; D=/viper/ptmp2/jinma/defaults_0923/cpu
BX=box_convection
...
for c in ref new; do
  ...
  $R t_vimp2_$c $c $BX 2 $D/slab2d_old.athinput time/tlim=50 problem/vpert=1.0e-2 rad_m1/implicit_vimp=true $X2 &
  ...
  $R t_spblast_$c $c sp_test 4 $W/inp/sp_blast.athinput time/nlim=100 &
done
wait
