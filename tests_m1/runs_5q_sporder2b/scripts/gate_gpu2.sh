#!/bin/bash -l
#SBATCH -o /viper/ptmp2/jinma/sporder2b_0925/gpu/log2.out.%j
#SBATCH -e /viper/ptmp2/jinma/sporder2b_0925/gpu/log2.err.%j
#SBATCH -J sporder2b
#SBATCH -p apudev
#SBATCH --ntasks=2
...
# m1-sp-order2b GPU (2): the Cartesian ref-vs-new differences of gate_gpu.sh (slab1,
...
run() {
  ...
  echo "== $t $c $(date +%T)"
  srun -n $np $W/bin/athena_${c%2}_${p}_gpu -d $d "$@" > log.txt 2>&1
  echo "rc=$?" >> log.txt
}
for c in ref new ref2; do
