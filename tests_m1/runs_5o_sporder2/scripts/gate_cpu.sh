#!/bin/bash -l
# m1-sp-order2 CPU gates: ref (7aabf6fd) vs new, the inputs of runs_5h_sph2
# (/viper/ptmp2/jinma/sph2_0924/inp, read only) plus copies with the new keys.
# usage: gate_cpu.sh [A|B|C|F|G|R ...]
W=/viper/ptmp2/jinma/sporder2_0925; I=/viper/ptmp2/jinma/sph2_0924/inp; J=$W/inp
...
G)  # the new keys NAMED with their default values: bitwise (payload after <par_end>)
  python3 $S/addkeys.py $I/sp_sph_atm_vc.athinput $J/g_vatm.athinput rad_m1 \
    implicit_marshak_face=cell vet_col_order2=false
  python3 $S/addkeys.py $I/sp_sph_sym_gas.athinput $J/g_symg.athinput rad_m1 \
    implicit_marshak_face=cell
  run G_vatm ref none 1 $I/sp_sph_atm_vc.athinput mesh/nx1=64 meshblock/nx1=64 time/nlim=300
  run G_vatm new none 1 $J/g_vatm.athinput mesh/nx1=64 meshblock/nx1=64 time/nlim=300
  run G_symg ref none 4 $I/sp_sph_sym_gas.athinput time/nlim=200
  run G_symg new none 4 $J/g_symg.athinput time/nlim=200
  wait
  for c in G_vatm G_symg; do cmpd $c; done;;
R)  # restart with the new options ON: straight vs restart at cycle 250, bitwise
  python3 $S/addkeys.py $I/sph_sym_gas_rst.athinput $J/r_rst.athinput rad_m1 \
    implicit_marshak_face=linear vet_col_order2=true
  run R_src new none 4 $J/r_rst.athinput rad_m1/c_light=100.0 rad_m1/closure=vet_col
  run R_srce new none 4 $J/r_rst.athinput rad_m1/c_light=100.0
  wait
  rrun R_rst new none 4 $W/gate/R_src/new/rst/sd.00001.rst
  rrun R_rste new none 4 $W/gate/R_srce/new/rst/sd.00001.rst
  wait
  python3 $S/rstcmp.py $W/gate/R_src/new $W/gate/R_rst/new
  python3 $S/rstcmp.py $W/gate/R_srce/new $W/gate/R_rste/new
  grep -h "NON-CONV\|vet_col_order2\|marshak" $W/gate/R_src/new/log.txt | head -4;;
esac; done
