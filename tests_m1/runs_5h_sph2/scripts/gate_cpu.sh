#!/bin/bash -l
# m1-sph2 CPU gates, ref (= d9583121 code, m1def2 fe147c9f binaries) vs new.
# usage: gate_cpu.sh [A|B|C|D|E ...]
W=/viper/ptmp2/jinma/sph2_0924; I=$W/inp
...
cmpd() { echo "== $1: $(python3 $W/scripts/gcmp.py $W/cpu/$1/ref $W/cpu/$1/new | tail -n1) $(tail -qn1 $W/cpu/$1/ref/log.txt $W/cpu/$1/new/log.txt | tr '\n' ' ')"; }
...
B)  # Cartesian, key absent (the current hesdirk2 + vimp defaults): bitwise
  for t in ref new; do
...
C)  # spherical-polar S1/S2/S5 + Cartesian vet_col with time_scheme = be named: bitwise
  for t in ref new; do
...
D)  # old restarts (ref binary, sp input without the key -> be) continued by ref and new
  run D_src ref none 4 $I/sph_sym_gas_rst.athinput rad_m1/c_light=100.0
  wait
  for t in ref new; do
    rrun D_old $t none 4 $W/cpu/D_src/ref/rst/sd.00001.rst
  done; wait; cmpd D_old
  python3 $W/scripts/rstcmp.py $W/cpu/D_src/ref $W/cpu/D_old/new
  grep -h "time_scheme" $W/cpu/D_old/new/log.txt | head -2;;
E)  # hesdirk2 on sp (the new default): restart bitwise, straight vs restart at 250
  run E_src new none 4 $I/sph_sym_gas_rst.athinput rad_m1/c_light=100.0
  run E_srcv new none 4 $I/sph_sym_gas_rst.athinput rad_m1/c_light=100.0 rad_m1/closure=vet_col
  wait
  rrun E_rst new none 4 $W/cpu/E_src/new/rst/sd.00001.rst
  rrun E_rstv new none 4 $W/cpu/E_srcv/new/rst/sd.00001.rst
  wait
  python3 $W/scripts/rstcmp.py $W/cpu/E_src/new $W/cpu/E_rst/new
  python3 $W/scripts/rstcmp.py $W/cpu/E_srcv/new $W/cpu/E_rstv/new
  grep -h "time_scheme=hesdirk2:" $W/cpu/E_src/new/log.txt $W/cpu/E_rst/new/log.txt $W/cpu/E_srcv/new/log.txt;;
esac; done
echo "GATES $* DONE"
