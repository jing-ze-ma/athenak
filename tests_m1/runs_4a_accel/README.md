...
| G0 GPU, switches off, bitwise vs d0c59f7c | PASS with bin_v5_gpu (md5 75a0e8e5, the final code apart from the fast_kernels vet-copy path and the od_skip -> fast_kernels rename): 3-D box, 40 cycles, be / hesdirk2 / hesdirk2+vet_sc, 39 bin/hst/rt_profile files identical (job 11953988). The same check with bin_v6_gpu (job 11954212) was still queued at commit time | `gpu_g0/`, `log.out.11953988` |
