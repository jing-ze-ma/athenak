...
for b in base new; do
  d=$W/gpu/g0b_$b; rm -rf $d; mkdir -p $d; cd $d
  srun -n 1 $W/$b/build_gpu/src/athena -i $W/inp/box3d_plm_vimp.athinput -d . $OV > run.log 2>&1
  echo "g0 $b rc=$?"
done
cd $W/gpu
for f in $(cd g0b_new && find . -name "*.bin" -o -name "*.hst" | sort); do
  cmp -s g0b_new/$f g0b_base/$f && echo "SAME $f" || echo "DIFF $f"
done
# --- G7
...
arm () {
  local c=$1 r=$2; local name=T7_${c}_$r
...
python3 $W/gpu/tools/summarize.py T7_
