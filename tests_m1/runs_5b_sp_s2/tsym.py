...
es = np.array([s2lib.load_tab(f)['m1_e'] for f in fs])
f1 = np.array([s2lib.load_tab(f)['m1_f1'] for f in fs])
f2 = np.array([s2lib.load_tab(f)['m1_f2'] for f in fs])
f3 = np.array([s2lib.load_tab(f)['m1_f3'] for f in fs])
e0 = s2lib.load_tab(fs[0].replace(f'.{num}.', '.00000.'))['m1_e']
t = s2lib.load_tab(fs[0])['time']
...
if hs:
    h = [s2lib.load_tab(f) for f in hs]
