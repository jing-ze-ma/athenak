...
COMMON="time/nlim=1 time/tlim=1.0e30 output1/dt=1.0e30 output2/dt=1.0e30
 ...
 problem/mlt_dump=mltfaces.txt problem/column_dump=column.txt time/cfl_number=1.0e-4"
...
run M0c "$BASE_D hydro/rad_tau_lo=1.0e5 hydro/rad_tau_hi=1.0e6 problem/rt_impl_mixed=0"
echo "##### ALL DONE"
# same reference at the NORMAL cfl, to separate the dt effect from the input edit
COMMON="${COMMON/ time\/cfl_number=1.0e-4/}"
run M0b "$BASE_D hydro/rad_tau_lo=1.0e5 hydro/rad_tau_hi=1.0e6 problem/rt_impl_mixed=0"
echo "##### ALL DONE"
