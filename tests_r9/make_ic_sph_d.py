...
HERE = '/viper/u2/jinma/ATHENAK/bench/hestar_presn'
...
OUT = '/viper/u2/jinma/ATHENAK/bench/wt_he4/tests_r9/ic_he4_presn_sph_tau1.txt'
...
def main():
    ...
    # tests_r9 arm (d): MOVE THE TAPER WINDOW UP, to tau = 1.0 / 0.10 instead of the
    # committed 3.0 / 0.30.  The four thresholds are re-read off THIS column at those two
    # optical depths, exactly as the committed pair was read off tau 3 / 0.3, so the ic
    # and the run's EOS still carry the same w.  Globals are rebound before the weights
    # are formed, and the numbers are printed for the input file.
    global RHO_HI, RHO_LO, T_HI, T_LO
    i_hi = int(np.argmin(np.abs(tau - 1.0)))
    i_lo = int(np.argmin(np.abs(tau - 0.10)))
    RHO_HI, RHO_LO = float(rho[i_hi]), float(rho[i_lo])
    T_HI, T_LO = float(Tt[i_hi]), float(Tt[i_lo])
    print('\n  ARM (d) WINDOW at tau 1.0 / 0.10:')
    print('    eos_rad_rho_hi = %.4e   (r = %.6e, r/R %.4f, tau %.4f)'
          % (RHO_HI, r[i_hi], r[i_hi]/2.3717e11, tau[i_hi]))
    print('    eos_rad_rho_lo = %.4e   (r = %.6e, r/R %.4f, tau %.4f)'
          % (RHO_LO, r[i_lo], r[i_lo]/2.3717e11, tau[i_lo]))
    print('    eos_rad_t_hi   = %.5e' % T_HI)
    print('    eos_rad_t_lo   = %.5e' % T_LO)
