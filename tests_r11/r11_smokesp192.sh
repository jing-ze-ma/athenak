...
A=$B/tests_r11/smokesp192; rm -rf $A; mkdir -p $A; cd $A
...
srun -n 2 $B/tests_r11/athena_v17 -i $B/inputs/hydro/he4_presn_sp.athinput $OV \
  time/nlim=40 mesh/nx2=192 mesh/nx3=192 meshblock/nx2=24 meshblock/nx3=24 output1/dt=1e30 output2/dt=1e30 output3/dt=1e30 \
  ...
  problem/cs_max=1.0e8 2>&1 | grep -E 'cycle=|FATAL|ADI|transverse|trs|Conduction|zone-cycles|cpu time|MeshBlocks|Error|error' | tail -25
echo "### exit $?"
