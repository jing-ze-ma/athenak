...
# current binary (athena_v7 = a6d66c3c + rt_top_vacuum plumbing), restart every half
...
A=$B/tests_r11/g3d; mkdir -p $A; cd $A
...
COMMON="output1/dt=47.0 output2/dt=2352.5 output3/dt=1.0e30 output4/dt=1.0e30
 output5/dt=47.0 problem/rt_profile_dt=47.0 problem/rt_surface_dt=94.0
 problem/column_dump=column_g3d.txt problem/mlt_dump=mltfaces_g3d.txt"
RST=$(ls -t $A/rst/*.rst 2>/dev/null | head -1)
if [ -n "$RST" ]; then
  srun -n 2 $B/tests_r11/athena_v7 -r $RST -i $B/inputs/hydro/he4_presn_cs.athinput \
       -t 03:50:00 $COMMON $OV time/tlim=23525.0
else
  srun -n 2 $B/tests_r11/athena_v7 -i $B/inputs/hydro/he4_presn_cs.athinput \
       -t 03:50:00 $COMMON $OV time/tlim=23525.0
fi
echo "### athena exit $?"
