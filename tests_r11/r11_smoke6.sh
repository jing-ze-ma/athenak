...
A=$B/tests_r11/${SMK:-smoke6}; rm -rf $A; mkdir -p $A; cd $A
...
srun -n 2 $B/tests_r11/athena_v21 -i $B/inputs/hydro/he4_presn_sp.athinput $OV \
  ...
  "$@" 2>&1 | grep -E 'cycle=|FATAL|ADI|transverse|trs|Conduction|zone-cycles|cpu time|MeshBlocks|Error|error' | tail -25
echo "### exit $?"
