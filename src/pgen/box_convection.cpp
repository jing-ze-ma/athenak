...
void ProblemGenerator::UserProblem(ParameterInput *pin, const bool restart) {
  ...
  if (rt_on_) {
    ...
    // THE HISTORY IS RESTART STATE.  It used to be the one thing a restart of this
    // problem generator did not reproduce: the warm start is the previous call's
    // converged Planck function, held per cell in two_stream_rt's rt_c3bp, so a cold
    // started first solve stops on a different one of the answers inside rt_impl_tol
    // (measured: 2e-15 in the mass, 2-5e-11 in the emergent flux, and the two runs
    // separate from there).  It is now written into the restart file behind the wtemp /
