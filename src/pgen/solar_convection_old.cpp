...
void ProblemGenerator::UserProblem(ParameterInput *pin, const bool restart) {
  ...
  Real igm1 = 1.0/gm1;
    ...
    KOKKOS_LAMBDA(int m, int k, int j, int i) {
      ...
      Real dden = denwb*0.0001*(sin(3.0*M_PI*x3v/(x3max-x3min))+cos(3.0*M_PI*x3v/(x3max-x3min)))*(sin(3.0*M_PI*x2v/(x2max-x2min))-cos(M_PI*x2v/(x2max-x2min)));
      Real tmpwb = pwb/denwb/Rgas;
      dden = (tmpwb > 6000.0 && pwb < 0.5*Rgas*rhobot*Tbot) ? dden : 0.0;
      Real p = pwb;
      Real den = denwb + dden;
...
void get_wb_Tp(const Real &p, Real &T) {
    ...
    Real rhobot = 1.0e-5;
    Real Rgas = 1.38e8;
    Real p0 = Rgas*Tbot*rhobot;
    Real p1 = 0.5*p0;
    Real Tsurf = 4000.0;

    Real Th = pow(p/p0,0.6)*Tbot;
    Real Tc = pow(p1/p0,0.6)*Tbot;
    Real Tm = pow(p/p1,0.39)*Tc;
    Real Tl = Tsurf;
    T = (Tm > Tl) ? Tm : Tl;
    T = (p > p1) ? Th : T;
