...
void DhjRtFreezeSplit(Mesh *pm, const Real dt) {
  ...
  if (!rt_test_out.empty()) {
      ...
      for (int k=ks; k<=ke; ++k) {
        for (int j=js; j<=je; ++j) {
          ...
          for (int i=ic; i<=ie; ++i) {
            ...
            sA += v*std::fabs(sd);
            sV += v*hdg(m,1,k,j,i);
            const Real rn = std::fabs(de - sd)/(hdg(m,1,k,j,i) + ts::ck_impl_norm_eps*emax);
    ...
    std::fprintf(rt_test_fp, "S %lld %.17e %.17e %lld %lld %.6e %d %d %.6e %.6e %d %d "
                 ...
                 static_cast<long long>(ndE), mxT, ts::ck_implicit ? ts::ck_impl_last_it : 0,
                 ts::ck_implicit ? ts::ck_impl_nonconv : 0, ts::ck_impl_last_res,
                 ts::ck_impl_last_gap, ts::ck_impl_ncap, ts::ck_impl_nfall, bud0, dE0, dS0,
                 budmax, rnmax, ve0);
    if (rt_test_ncall == 1 || rt_test_ncall % rt_test_every == 0) {
      const char tag[3] = {'T', 'E', 'C'};
      for (int q=0; q<3; ++q) {
        std::fprintf(rt_test_fp, "%c %lld", tag[q], static_cast<long long>(rt_test_ncall));
        for (int i=is; i<=ie; ++i) std::fprintf(rt_test_fp, " %.17e",
                                                hdg(m0,q,k0,j0,i));
        std::fprintf(rt_test_fp, "\n");
      }
