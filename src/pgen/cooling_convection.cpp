...
//========================================================================================
//! \file cooling_convection.cpp
//! \brief Solar surface convection started from a SUB-ADIABATIC (convectively stable)
//!        stratification, to test whether surface radiative cooling alone can drive the
//!        stable layer convective.
//!
//! Differs from solar_convection.cpp only in the initial stratification: the interior is
//! laid down at nabla = grad_int < nabla_ad instead of on the adiabat, so N^2 > 0 and the
//! whole box is Schwarzschild-stable at t=0. The only buoyancy source is then the entropy
//! deficit created by radiative cooling at the photosphere.
//!
//! REFERENCE:
...
void get_kapr(const Real &rho, const Real &T, const Real &kap0, const Real &kapa, const Real &kapb, Real &kapr);
...
// too).  The open bottom boundary and the CO5BOLD inflow-entropy relaxation read these
// instead of hard-coded literals, so the boundary entropy cannot drift out of sync with the
// IC when grad_int or the opacity changes -- that mismatch silently injected a large entropy
// excess at the bottom in solar_convection.cpp.
namespace {
Real p_base_ic   = 0.0;
Real rho_base_ic = 0.0;
Real T_base_ic   = 0.0;
Real grad_int_ic = 0.0;
// <problem>/kappa_const: if > 0, use a CONSTANT opacity everywhere instead of the composite
// H-/Kramers/scattering law.  <problem>/radiative_ic: if true, build the IC in radiative
// equilibrium all the way down (no Schwarzschild switch to an adiabat).  Together these give
// an IC that is simultaneously in genuine radiative equilibrium AND convectively stable
// (nabla -> 1/4), which the H- law cannot produce at any normalisation -- the controlled test
// of whether surface cooling alone can drive convection.
Real kappa_const_ic = 0.0;
Real kappa_a_ic     = 0.0;
Real kappa_b_ic     = 0.0;
bool radiative_ic   = false;
}  // namespace
...
void ProblemGenerator::UserProblem(ParameterInput *pin, const bool restart) {
  ...
  // Sub-adiabatic interior gradient and the resulting base state.  Done BEFORE the restart
  // return because the bottom boundary condition needs the base state on restarts as well.
  // grad_int defaults to the true adiabatic gradient, which reproduces solar_convection.cpp
  // exactly; set <problem>/grad_int below (gamma-1)/gamma for a convectively STABLE start.
  {
    ...
    kappa_b_ic = pin->GetOrAddReal("problem","kappa_b",0.0);
    radiative_ic = pin->GetOrAddBoolean("problem","radiative_ic",false);
    Real Rgas_ic = 1.38e8;
    Real grav_ic = -2.74e4;
    ...
    if (global_variable::my_rank == 0) {
      std::cout << "cooling_convection: grad_int=" << grad_int_ic
                << " kappa=" << kappa_const_ic << "*rho^" << kappa_a_ic << "*T^" << kappa_b_ic
                << " radiative_ic=" << radiative_ic
                << " (nabla_ad=" << (gamma_ic-1.0)/gamma_ic << ")"
                << "  base state: T=" << T_base_ic << " rho=" << rho_base_ic
                << " p=" << p_base_ic
  ...
  Real igm1 = 1.0/gm1;
    ...
    // White-noise seed amplitude. Deliberately WEAK here (1e-4, vs 1e-2 in solar_convection):
    // this problem asks whether surface cooling can destabilise a stable layer on its own, so
    // the seed must only break the symmetry, not supply the buoyancy. In a sub-adiabatic
    // interior the perturbations do not grow anyway -- they just ring as g-modes.
    Real seed_amp = pin->GetOrAddReal("problem","seed_amp",1.0e-4);
...
void HydrostaticEquilibrium(Mesh *pm) {
  ...
  auto &size = pmbp->pmb->mb_size;
    ...
    Real Rgas_bc = 1.38e8;
    // Base state taken from the IC integration (see the top of this file), NOT hard-coded:
    // it must track grad_int and the opacity, or the boundary injects a spurious entropy
    // excess.  Copied into locals here so the device lambdas below capture by value.
    Real p_base  = p_base_ic;
    Real T_base  = T_base_ic;
    Real rho_base = rho_base_ic;
...
//        u0(m,IM3,k,j,i) -= u0(m,IM3,k,j,i)*fredux;
    ...
    // from the vertical velocity (zero net mass flux). Horizontal means are GLOBAL over the plane.
    {
      Real igm1 = 1.0/gm1;
      Real cv = Rgas*igm1;
      // Inflow entropy is anchored to the IC base state (see the top of this file) so that it
      // MATCHES the initial stratification at t=0.  With a sub-adiabatic IC the base entropy
      // is lower than the photospheric adiabat, and hard-coding the old adiabatic values here
      // would make the bottom inject high-entropy fluid and drive the convection itself --
      // exactly the confound this experiment has to avoid.  The bottom then supplies energy
      // only once surface-cooled downflows actually arrive with s < s_inflow.
...
//   * Interior: once the radiative gradient exceeds nabla_ad (the Schwarzschild threshold,
//     grad_switch), lay the stratification down at grad_int instead of following the
//     radiative gradient: T = T_sw (p/p_sw)^grad_int.
// The two gradients are deliberately SEPARATE.  grad_switch = (gamma-1)/gamma is physics: it
// is where radiation can no longer carry the flux, and it fixes the photosphere depth.
// grad_int is the gradient actually imposed below that point.  grad_int = grad_switch gives
// the usual neutrally-stable adiabat; grad_int < grad_switch gives a SUB-adiabatic, fully
// Schwarzschild-stable interior (N^2 > 0) while leaving the photosphere where it was.
...
void get_wb_eos_arr(const EOS_Data &eos, const Real &Rgas, const Real &grav_acc, const Real &gamma, const Real &grad_int, const Real &kap0, const Real &kapa, const Real &kapb, const bool &rad_only, const int &N, const Real &zmax, View1D zarr, View1D logparr, View1D Tarr) {
    ...
    Real grad_ad = grad_int;                  // gradient imposed on the interior
    ...
    auto integrate = [&](Real p_top, bool store) -> Real {
        ...
        for (int n=N-1; n>=0; --n) {
            ...
            Real T_new;
            if (rad_only) {
                // Radiative equilibrium ALL the way down: T follows the gray Eddington
                // relation at every depth, no Schwarzschild switch. Only sensible with a
                // constant (or weakly depth-dependent) opacity -- with the H- law this
                // drives nabla up to ~4 and INVERTS the density.
                T_new = Teff*std::pow(0.75*(tau_new+2.0/3.0), 0.25);
            } else if (!convective) {
...
KOKKOS_INLINE_FUNCTION
void get_kapr(const Real &rho, const Real &T, const Real &kap0, const Real &kapa, const Real &kapb, Real &kapr) {
  // kap0 > 0 selects a CONSTANT opacity instead of the composite law below.  That is not a
  // cosmetic change: with kappa constant the gray Eddington radiative-equilibrium profile has
  // nabla -> 1/4 < nabla_ad, i.e. it is convectively STABLE at every depth, which the H- law
  // can never be.  Note that merely rescaling the H- normalisation does NOT work --
  // nabla_rad = kappa*p/(4g(tau+2/3)) and p ~ g*tau/kappa, so the product kappa*p (and hence
  // nabla_rad) is INDEPENDENT of the normalisation.  The instability comes from kappa RISING
  // with depth (rho^0.5 T^9), so the temperature exponent is the lever, not the prefactor.
  if (kap0 > 0.0) {
    // Power law kap0*(rho/1e-6)^kapa*(T/1e4)^kapb.  kapa=kapb=0 gives a constant opacity.
    // The T exponent is what decides stability: with rho^0.5, the radiative-equilibrium
    // profile stays sub-adiabatic only for kapb < ~0.9 (max nabla 0.326 at kapb=0, 0.393 at
    // 0.8, 0.414 -- unstable -- at 1.0).  The real H- law has kapb=9, hence its instability.
    kapr = kap0*pow(rho/1.0e-6, kapa)*pow(T/1.0e4, kapb);
    return;
  }
  Real T4 = T/1.0e4;
...
void two_stream_RT(Mesh *pm, Real bdt) {
    ...
    Real cgs2Pa = 0.1;
    Real boltz_sigma = 5.6704e-5;
    Real kap0 = kappa_const_ic;   // local copies: device lambdas capture by value, and a device
    Real kapa = kappa_a_ic;       // kernel cannot read a host-side global directly
    Real kapb = kappa_b_ic;
    ...
    KOKKOS_LAMBDA(const int m, const int k, const int j) {
        ...
        Real kapr;
        get_kapr(rho, T, kap0, kapa, kapb, kapr);
