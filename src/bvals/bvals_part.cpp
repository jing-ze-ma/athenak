...
//  sendlist.template sync<DevExeSpace>();
    sendlist.modify_host();
    sendlist.sync_device();
...
TaskStatus ParticlesBoundaryValues::PackAndSendPrtcls() {
  ...
  if (nprtcl_send > 0) {
    ...
    auto sendlist_ = sendlist;   // alias: naming the member captures `this`
...
//  sendlist.template sync<DevExeSpace>();
  ...
  if (nprtcl_recv > 0) {
    ...
    int &npart = pmy_part->nprtcl_thispack;
    auto sendlist_ = sendlist;   // alias: naming the member captures `this`
    auto nprtcl_send_ = nprtcl_send;   // alias: naming the member captures `this`
