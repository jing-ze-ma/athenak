...
void SingleC2P_IdealMHD(MHDCons1D &u, const EOS_Data &eos,
  ...
  if (eos.vceil > 0.0) {
    ...
    if (vsq > SQR(eos.vceil)) {
      ...
      if (!eos.defer_cons_floors) {
        ...
        // <mhd>/vceil_thermalise: leave the TOTAL energy alone, so that w.e below picks
        // the clipped kinetic energy up as internal energy (see EOS_Data).
  ...
  if (w.e < efloor) {
    w.e = efloor;
    if (!eos.defer_cons_floors) u.e = efloor + e_k + e_m;
    efloor_used = true;
  }
