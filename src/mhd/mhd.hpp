...
class MHD {
 public:
  ...
  // Mirror of Hydro::wb_rmax; see the note there.
  Real wb_rmax = 0.0;
  ...
  TaskStatus ImplicitTransverseConduction(Driver *d, int stage);
      ...
      for (int n=0; n<nvar; ++n) {
          if ((n == (IEN) || (n == (IDN) && use_wb_rho)) && use_wellbalance_dynamic && use_wb_x1 && hyd) {
            par_for_inner(member, il, iu, [&](const int i) {
              ...
              // MHD::wb_rmax / MHD::wb_rmin), so this cell reconstructs its state
