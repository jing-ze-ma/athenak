#ifndef MHD_RSOLVERS_HLLD_UCT_MHD_HPP_
#define MHD_RSOLVERS_HLLD_UCT_MHD_HPP_
//========================================================================================
...
//! - A. Mignone & L. Del Zanna, "Systematic construction of upwind constrained transport
//!   schemes for MHD", JCP, 424, 109748 (2021)

namespace mhd {

// The tolerance for treating Bx as zero when deciding whether the rotational
// discontinuities (the ** states) exist is now the runtime parameter
// EOS_Data::hlld_bx_zero_tol (<mhd>/hlld_bx_zero_tol, default 1.0e-4, the historical
// AthenaK/Athena++ value -- see the note on that field in eos.hpp).  This test compares
// 0.5*Bx^2 to the TOTAL pressure pt*, so it is a plasma-beta cut: a tolerance of tol
// drops the ** states for every beta above ~2/tol regardless of how dynamically strong
// the field is.  At beta above ~2e4 with Alfven Mach number <~ 1 the default is
// unstable (the Balsara advected-vortex test grows its kinetic and magnetic energies
// 100-1000x with no NaN); setting the parameter down to ~1.0e-8 restores the ** states
// and fixes that, at the cost of extra numerical dissipation of the transverse field in
// very-high-beta, dynamically weak regimes (e.g. deep hot-Jupiter envelopes), where the
// low value is the wrong choice.  Kept separate from HLLD_SMALL_NUMBER, whose other two
// uses compare |rho sd sdm - Bx^2| ~ rho c_f^2, a different and unrelated scale.

//----------------------------------------------------------------------------------------
//! \fn

KOKKOS_INLINE_FUNCTION
void HLLD_UCT(TeamMember_t const &member, const EOS_Data &eos,
     const RegionIndcs &indcs,const DualArray1D<RegionSize> &size,const CoordData &coord,
     const int m, const int k, const int j, const int il, const int iu, const int ivx,
     const ScrArray2D<Real> &wl, const ScrArray2D<Real> &wr,
     const ScrArray2D<Real> &bl, const ScrArray2D<Real> &br, const DvceArray4D<Real> &bx,
     DvceArray5D<Real> flx, DvceArray4D<Real> ey, DvceArray4D<Real> ez, DvceArray4D<Real> laml, DvceArray4D<Real> lamr, DvceArray4D<Real> vxl, DvceArray4D<Real> vxr, DvceArray4D<Real> vyl, DvceArray4D<Real> vyr, DvceArray4D<Real> vzl, DvceArray4D<Real> vzr, DvceArray4D<Real> nus, DvceArray4D<Real> dl, DvceArray4D<Real> dr) {
  ...
  if (eos.is_ideal) {
    ...
    par_for_inner(member, il, iu, [&](const int i) {
      ...
      if (0.5*bxsq < eos.hlld_bx_zero_tol*ptst) {
        uldst = ulst;
        urdst = urst;
      } else {
      ...
      Real nustar = (fabs(spd[3]-spd[1]) > 1.0e-9*fabs(spd[4]-spd[0])) ? ((fabs(spd[3])-fabs(spd[1]))/(spd[3]-spd[1])) : 0.0;
      Real nul = (spd[1]+spd[0])/(fabs(spd[1])+fabs(spd[0]));
      Real nur = (spd[3]+spd[4])/(fabs(spd[3])+fabs(spd[4]));
      Real chitl = (wl_ivx-spd[2])*(spd[0]-spd[2])/(spd[1]+spd[0]-2.0*spd[2]);
      Real chitr = (wr_ivx-spd[2])*(spd[4]-spd[2])/(spd[3]+spd[4]-2.0*spd[2]);
      laml(m,k,j,i) = spd[0];
      lamr(m,k,j,i) = spd[4];
      nus(m,k,j,i) = (fabs(spd[4])-fabs(spd[0]))/(spd[4]-spd[0]); // nustar;
      Real alpr = fmax(0.0,spd[4]);
      Real alpl = -fmin(0.0,spd[0]);
      dl(m,k,j,i) = alpr*alpl/(alpr+alpl); // 0.5*(nul-nustar)*chitl + 0.5*(fabs(spd[1])-nustar*spd[1]);
      dr(m,k,j,i) = alpr*alpl/(alpr+alpl); // 0.5*(nur-nustar)*chitr + 0.5*(fabs(spd[3])-nustar*spd[3]);
    });
  ...
  //------------------------- ISOTHERMAL HLLD solver -------------------------------------
    ...
    par_for_inner(member, il, iu, [&](const int i) {
      ...
      Real chitr = (wr_ivx-spd[2])*(spd[4]-spd[2])/(spd[3]+spd[4]-2.0*spd[2]);
      nus(m,k,j,i) = nustar;
      dl(m,k,j,i) = 0.5*(nul-nustar)*chitl + 0.5*(fabs(spd[1])-nustar*spd[1]);
      dr(m,k,j,i) = 0.5*(nur-nustar)*chitr + 0.5*(fabs(spd[3])-nustar*spd[3]);
    });
...
#endif // MHD_RSOLVERS_HLLD_UCT_MHD_HPP_
