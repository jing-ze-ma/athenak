...
void Conduction::ImplicitTransverseUpdate(DvceArray5D<Real> &u0, const EOS_Data &eos,
  ...
  // ---- THE LINEARISATION POINT.  BuildAngularCoeffs filled T*_i and alpha_i from w0 at
  // the START of the stage, i.e. before RKUpdate, the source terms and -- crucially --
  // ImplicitRadialUpdate, which is the operator doing the largest work per stage in the
  // optically thick rows just above the bottom wall.  The increment y this routine
  // computes is then added to the POST-radial energy.  That lag is not a small error: it
  // makes the operator drive the energy towards transverse temperature differences the
  // radial solve has already removed, i.e. ANTI-diffusive relative to the state it acts
  // on, and since the result feeds the next stage's w0 the error is amplified every
  // cycle.  MEASURED in the 3 Msun He FeCZ box: max |de| grew 1.4-1.5x per stage from
  // cycle 1 and reached T = 1.9e14 K in the second active row by cycle 36.
  //
  // So T*_i and alpha_i are re-evaluated here, from the CURRENT conserved state, with
  // exactly the extraction ConToPrim and ImplicitRadialUpdate use (EintFromCons: the
  // gravitational term, the cubed-sphere kinetic energy, the magnetic energy).  The FACE
  // conductances C_f stay frozen at w0 -- they are a coefficient of the linear system,
  // not its state, and rebuilding them needs the whole opacity path.
  //
  // GHOSTS.  u0 in the x2/x3 ghosts is stale (the exchange happens after this task), so
  // the refreshed values have to be communicated before the stencil reads them: both
  // cells of a face must form the SAME T and alpha for their shared neighbour or the
  // flux form stops being antisymmetric and conservation is lost.  The two registers
  // tr_ya/tr_yb are free until the RKL1 loop zeroes them below, so they carry the
  // exchange; a ghost outside a physical x2/x3 boundary keeps its w0 value and is never
  // read, since that face is closed.
  if (rad_ang_refresh || rad_ang_verbose) {
    auto ta_ = tr_ya;
    auto aa_ = tr_yb;
    const int n1 = st.extent_int(4), n2 = st.extent_int(3), n3 = st.extent_int(2);
    par_for("radtrref0", DevExeSpace(), 0, nmb1, 0, n3-1, 0, n2-1, 0, n1-1,
    KOKKOS_LAMBDA(const int m, const int k, const int j, const int i) {
      ta_(m,0,k,j,i) = st(m,it_,k,j,i);
      aa_(m,0,k,j,i) = st(m,ia_,k,j,i);
    });
    const bool ismhd = (my_block.compare("mhd") == 0);
    auto &wtemp_ = ismhd ? pmy_pack->pmhd->wtemp : pmy_pack->phydro->wtemp;
    auto &phicc_ = ismhd ? pmy_pack->pmhd->phicc0 : pmy_pack->phydro->phicc0;
    ...
    auto eos_ = eos;
    const bool gen = eos.IsGeneral();
    // the diagnostic: how far the frozen linearisation point has drifted from the state
    // the increment is actually applied to, and where.  Costs one extra reduction and is
    // only formed when <hydro>/rad_ang_verbose is set.
    const bool diag = rad_ang_verbose;
    Kokkos::MaxLoc<Real, int>::value_type dloc;
    dloc.val = -1.0;
    dloc.loc = -1;
    const int nx1_ = indcs.nx1, nx2_ = indcs.nx2, nx3_ = indcs.nx3;
    const int nkji_ = nx3_*nx2_*nx1_, nji_ = nx2_*nx1_;
    Kokkos::parallel_reduce("radtrref",
    Kokkos::RangePolicy<>(DevExeSpace(), 0, (nmb1 + 1)*nkji_),
    KOKKOS_LAMBDA(const int &idx, Kokkos::MaxLoc<Real, int>::value_type &dres) {
      const int m = idx/nkji_;
      const int k = (idx - m*nkji_)/nji_ + ks;
      const int j = (idx - m*nkji_ - (k - ks)*nji_)/nx1_ + js;
      const int i = (idx - m*nkji_ - (k - ks)*nji_ - (j - js)*nx1_) + is;
      ...
      const Real ei = EintFromCons(u0, m, k, j, i, cs_ ? cosc_(m,k,j) : 0.0, cs_, etg,
                                   etg ? phicc_(m,k,j,i) : 0.0,
                                   ismhd ? MagEnergyCC(bcc_,m,k,j,i) : 0.0);
      // a cell whose internal energy is not positive is dropped from the system (alpha
      // = 0 on both sides of each of its faces) rather than handed to the EOS inversion
      Real tt = 0.0, al = 0.0;
      if (ei > 0.0 && d > 0.0 && isfinite(ei)) {
        tt = eos_.Temperature(d, ei, gen ? wtemp_(m,k,j,i) : -1.0);
        if (tt > 0.0 && isfinite(tt)) {
          const Real cv = eos_.SpecificHeatCv(d, ei, tt);
          if (cv > 0.0 && isfinite(cv)) al = 1.0/(d*cv);
        }
      }
      if (!(al > 0.0)) {
        tt = st(m,it_,k,j,i);
      ...
      aa_(m,0,k,j,i) = al;
      if (diag) {
        const Real told = st(m,it_,k,j,i);
        const Real rd = (told > 0.0) ? fabs(tt - told)/told : 0.0;
        if (isfinite(rd) && rd > dres.val) { dres.val = rd; dres.loc = idx; }
      }
    }, Kokkos::MaxLoc<Real, int>(dloc));
    if (rad_ang_verbose && ang_lines < 400 && dloc.loc >= 0) {
      auto rec = cap_rec;
      const int dl = dloc.loc;
      par_for("radtrrefrec", DevExeSpace(), 0, 0, KOKKOS_LAMBDA(const int) {
        const int m = dl/nkji_;
        const int k = (dl - m*nkji_)/nji_ + ks;
        const int j = (dl - m*nkji_ - (k - ks)*nji_)/nx1_ + js;
        const int i = (dl - m*nkji_ - (k - ks)*nji_ - (j - js)*nx1_) + is;
        rec(0) = static_cast<Real>(k);
        rec(1) = static_cast<Real>(j);
        rec(2) = static_cast<Real>(i);
        rec(3) = ta_(m,0,k,j,i);     // T from the CURRENT conserved state
        rec(4) = st(m,it_,k,j,i);    // T* as BuildAngularCoeffs froze it from w0
        rec(5) = static_cast<Real>(m);
      });
      auto hrec = Kokkos::create_mirror_view(cap_rec);
      Kokkos::deep_copy(hrec, cap_rec);
      std::cout << "### rad_ang_refresh cycle " << pmy_pack->pmesh->ncycle
                << " rank " << global_variable::my_rank
                << ": max |T(cons)/T*(w0) - 1| = " << dloc.val
                << " at (m,k,j,i) = (" << static_cast<int>(hrec(5)) << ","
                << static_cast<int>(hrec(0)) << "," << static_cast<int>(hrec(1)) << ","
                << static_cast<int>(hrec(2)) << "), T(cons) = " << hrec(3)
                << ", T*(w0) = " << hrec(3)/(1.0 + 0.0) << std::endl;
    if (rad_ang_refresh) {
    // both halves of every shared face must see identical numbers: exchange them
    pbval_tr->InitRecv(1);
    pbval_tr->PackAndSendCC(ta_, tr_ycoar);
    while (pbval_tr->RecvAndUnpackCC(ta_, tr_ycoar) != TaskStatus::complete) {}
    while (pbval_tr->ClearRecv() != TaskStatus::complete) {}
    while (pbval_tr->ClearSend() != TaskStatus::complete) {}
    pbval_tr->InitRecv(1);
    pbval_tr->PackAndSendCC(aa_, tr_ycoar);
    while (pbval_tr->RecvAndUnpackCC(aa_, tr_ycoar) != TaskStatus::complete) {}
    while (pbval_tr->ClearRecv() != TaskStatus::complete) {}
    while (pbval_tr->ClearSend() != TaskStatus::complete) {}
    auto stw = tr_st;
    par_for("radtrref1", DevExeSpace(), 0, nmb1, 0, n3-1, 0, n2-1, 0, n1-1,
    KOKKOS_LAMBDA(const int m, const int k, const int j, const int i) {
      stw(m,it_,k,j,i) = ta_(m,0,k,j,i);
      stw(m,ia_,k,j,i) = aa_(m,0,k,j,i);
    });
    }
    }
  }
