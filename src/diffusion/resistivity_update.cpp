...
TaskStatus Resistivity::RKUpdate(Driver *pdriver, int stage) {
  ...
  auto u2_ = u2;
  auto u_ideal_ = u_ideal;
  auto flx1 = pmy_pack->pmhd->uflx.x1f;
  auto flx2 = pmy_pack->pmhd->uflx.x2f;
  auto flx3 = pmy_pack->pmhd->uflx.x3f;
  auto flx1_ideal = uflx_ideal.x1f;
  auto flx2_ideal = uflx_ideal.x2f;
  auto flx3_ideal = uflx_ideal.x3f;
  ...
  KOKKOS_LAMBDA(TeamMember_t member, const int m, const int k, const int j) {
    ...
    if (use_cubed_sphere || use_spherical_polar) {
      ...
      par_for_inner(member, is, ie, [&](const int i) {
        divf(i) = ((fjm1*flx1(m,n,k,j,i+1)+f0*flx1_ideal(m,n,k,j,i+1))*area1(m,k,j,i+1) - (fjm1*flx1(m,n,k,j,i)+f0*flx1_ideal(m,n,k,j,i))*area1(m,k,j,i))/volume(m,k,j,i);
      });
      member.team_barrier();
      ...
      if (multi_d) {
        par_for_inner(member, is, ie, [&](const int i) {
          divf(i) += ((fjm1*flx2(m,n,k,j+1,i)+f0*flx2_ideal(m,n,k,j+1,i))*area2(m,k,j+1,i) - (fjm1*flx2(m,n,k,j,i)+f0*flx2_ideal(m,n,k,j,i))*area2(m,k,j,i))/volume(m,k,j,i);
      });
      member.team_barrier();
      }
      ...
      if (three_d) {
        par_for_inner(member, is, ie, [&](const int i) {
          divf(i) += ((fjm1*flx3(m,n,k+1,j,i)+f0*flx3_ideal(m,n,k+1,j,i))*area3(m,k+1,j,i) - (fjm1*flx3(m,n,k,j,i)+f0*flx3_ideal(m,n,k,j,i))*area3(m,k,j,i))/volume(m,k,j,i);
        });
        member.team_barrier();
      }
    ...
    // compute dF1/dx1
    par_for_inner(member, is, ie, [&](const int i) {
      divf(i) = ((fjm1*flx1(m,n,k,j,i+1)+f0*flx1_ideal(m,n,k,j,i+1)) - (fjm1*flx1(m,n,k,j,i)+f0*flx1_ideal(m,n,k,j,i)))/mbsize.d_view(m).dx1;
    });
    member.team_barrier();
    ...
    if (multi_d) {
      par_for_inner(member, is, ie, [&](const int i) {
        divf(i) += ((fjm1*flx2(m,n,k,j+1,i)+f0*flx2_ideal(m,n,k,j+1,i)) - (fjm1*flx2(m,n,k,j,i)+f0*flx2_ideal(m,n,k,j,i)))/mbsize.d_view(m).dx2;
      });
      member.team_barrier();
    }
    ...
    if (three_d) {
      par_for_inner(member, is, ie, [&](const int i) {
        divf(i) += ((fjm1*flx3(m,n,k+1,j,i)+f0*flx3_ideal(m,n,k+1,j,i)) - (fjm1*flx3(m,n,k,j,i)+f0*flx3_ideal(m,n,k,j,i)))/mbsize.d_view(m).dx3;
      });
      member.team_barrier();
    }
