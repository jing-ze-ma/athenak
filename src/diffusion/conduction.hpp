...
class Conduction {
 public:
  ...
  // Still symmetric, still an M-matrix.  Every geometric factor of the T-form carries
  // over untouched: the curvilinear face area and the (stretched) radial width sit
  // inside C_f, and the curvilinear cell volume sits in the diagonal, so the u-form is
  // the same operator on the spherical-polar and cubed-sphere grids the red giant and
  // the deep hot Jupiter run on.
  //
  ...
  // so each pass CORRECTS the step already taken instead of taking another full one.
