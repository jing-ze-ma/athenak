#!/bin/bash -l
# usage: build.sh <commit> <tag> <cpu|gpu> <jobs>   (git archive snapshot of wt_ckrst)
set -e
...
if [ ! -d $S ]; then
  mkdir -p $S; git -C /viper/ptmp2/jinma/wt_ckrst archive $C | tar -x -C $S
