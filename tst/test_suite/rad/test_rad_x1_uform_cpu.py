...
src/diffusion/conduction.cpp), on the operator this branch actually has: the implicit
tridiagonal x1 solve with the TRANSVERSE (x2/x3) faces carried EXPLICITLY in the ordinary
face fluxes.  The problem is the spreading anisotropic Gaussian of
inputs/tests/rad_x1_uform_gauss.athinput (pgen rad_diff2d).
...
WHAT THIS GUARDS.
  * both forms converge at the 2nd order of the stencil, and to the same solution: this
    ...
    where the two forms are supposed to coincide, and they do to 3e-6 in L1 -- which is
    the precision the error file is written at, so it is a null.  A face recipe that
    drifted between the forms -- a different T_f, a factor 4 out of place, an unscaled
    diagonal -- would show up here first and as a large effect;
  ...
  * and with both switches at their defaults the solve is untouched.

THE TIMESTEP.  Unlike the super-time-stepped variant of this problem, the transverse
direction here is explicit, so dt is set by the transverse diffusion limit and already
falls as dx^2 at FIXED cfl_number; cfl_number is therefore held fixed across the two
resolutions rather than scaled with 1/n.  The hydro CFL sits four orders of magnitude
above it at both resolutions and never binds.
...
# cfl_number, held FIXED at both resolutions: dt is the EXPLICIT TRANSVERSE conduction
# limit (9.38e-7 at nx = 32, 4x smaller at nx = 64, against a hydro CFL of 9.7e-3), which
# scales as dx^2 on its own.
CFL = 0.4

# L1 error of the total energy against the analytic solution.  MEASURED on CPU:
#   T-form (both switches off)  2.262905e-08 (32)  5.290219e-09 (64)  order 2.097
#   u-form, kiter 1             2.262902e-08       5.290212e-09       order 2.097
#   u-form, kiter 2             2.262900e-08       5.290206e-09       order 2.097
# The gates carry 15 % of margin.
L1_MAX = {32: 2.61e-8, 64: 6.10e-9}
ORDER_MIN = 1.95
...
# Measured: 1.3e-6 (kiter 1) and 2.5e-6 (kiter 2) in relative L1, i.e. the last digit of
# the 7-figure error file.
AGREE_MAX = 1.0e-4
# |sum V de - expected|/sum V|de| of the solve.  Interior faces only, flux form: round-off
# (measured 2.3e-16 for the T-form and 2.4e-16 for the u-form).
CONS_MAX = 1.0e-12
# ...and for a second Picard pass, whose own increments are ~1e-8 of the first pass's, so
# that the same ratio is round-off over round-off (measured 1.2e-8).
...
def run(binary, arm, n):
    ...
    args = ["mesh/nx1=%d" % n, "mesh/nx2=%d" % n,
            ...
            "time/cfl_number=%.10g" % CFL] + ARMS[arm]
