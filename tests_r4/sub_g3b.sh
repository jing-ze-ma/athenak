#!/bin/bash -l
#SBATCH -o /viper/u2/jinma/ATHENAK/bench/wt_he4/tests_r4/g3b_TAG.log
#SBATCH -J g3b
#SBATCH -p apudev
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
for row in "${CASES[@]}"; do
  ...
  srun -n 1 $EXE -i $IN3 mesh/x1max=$x1max problem/kappa_const=$kappa \
    ...
    problem/rt_implicit_column=MD problem/rt_col3_skip_sweep=SKIP \
    < /dev/null > run.log 2>&1
  echo "=== G3 TAG $tag exit $?"
done
echo ALLDONE
